...
@section('content')
    <div class="bg-white rounded shadow-sm px-6 py-4 mb-4">
        ...
        <div class="grid sm:grid-cols-2 sm:gap-4">
            <div>
                <div class="flex mb-2">
                    <p class="w-1/3">{{ __('Booking Number') }}</p>
                    <p class="text-gray-600">{{ $booking->booking_number }}</p>
                </div>
                <div class="flex mb-2">
                    <p class="w-1/3">{{ __('Customer') }}</p>
        ...
        <table class="table-auto w-full mb-4">
            ...
            @forelse ($booking->bookingGoods as $index => $bookingItem)
                <tr class="{{ $index % 2 == 0 ? 'bg-gray-100' : '' }}">
                    <td class="px-4 py-1 text-center">{{ $index + 1 }}</td>
                    <td class="px-4 py-1">
                        <a href="{{ route('goods.show', ['goods' => $bookingItem->goods->id]) }}" class="text-link">
                            {{ $bookingItem->goods->item_name }}
                        </a>
                    </td>
                    <td class="px-4 py-1">{{ $bookingItem->goods->item_number }}</td>
