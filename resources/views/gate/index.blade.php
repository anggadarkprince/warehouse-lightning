@extends('layouts.app')

@section('content')
    <div class="bg-white rounded shadow py-4 mb-4">
        <div class="flex justify-between items-center mb-3 px-6">
            <div>
                <h1 class="text-xl text-green-500">Gate</h1>
                <span class="text-gray-400">Job management & data service</span>
            </div>
        </div>
        <div class="px-6">
            <form action="{{ route('gate.index') }}" method="get">
                <div class="flex flex-wrap mb-3 sm:mb-4">
                    <div class="flex w-full">
                        <input type="text" name="code" id="code" class="form-input rounded-tr-none rounded-br-none" placeholder="Input booking, delivery order or work order" aria-label="qr-code">
                        <button type="submit" class="button-light relative py-2 px-4 rounded-none border border-transparent">
                            <i class="mdi mdi-qrcode-scan"></i>
                        </button>
                        <button type="submit" class="button-primary relative py-2 px-4 rounded-tl-none rounded-bl-none border border-transparent">
                            Submit
                        </button>
                    </div>
                </div>
            </form>
        </div>
    </div>
@endsection
