    ...
    <div class="bg-white rounded shadow px-6 py-4">
        ...
        <table class="table-auto w-full mb-4">
            ...
            <tr>
                ...
                <th class="border-b border-t px-4 py-2 text-left">Role</th>
            ...
            @forelse ($roles as $index => $role)
                <tr class="{{ $index % 2 == 0 ? 'bg-gray-100' : '' }}">
                    ...
                    <td class="px-4 py-1">{{ $role->role }}</td>
                    <td class="px-4 py-1">{{ $role->permission_total }}</td>
                    <td class="px-4 py-1">{{ $role->description ?: '-' }}</td>
                    <td class="px-4 py-1">{{ optional($role->created_at)->format('d F Y H:i') }}</td>
