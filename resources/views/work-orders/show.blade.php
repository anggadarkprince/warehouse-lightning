...
@section('content')
    <div class="bg-white rounded shadow-sm px-6 py-4 mb-4">
        <div class="mb-2">
            ...
            <span class="text-gray-400">{{ __('Manage job data') }}</span>
        </div>
        ...
        <div class="bg-white rounded shadow-sm px-6 py-4 mb-4">
            <div class="mb-2 flex items-center justify-between">
                <div>
                    ...
                    <span class="text-gray-400">{{ __('List of job containers') }}</span>
                </div>
            </div>
            <table class="table-auto w-full mb-4">
                ...
                @forelse ($workOrder->workOrderContainers as $index => $workOrderContainer)
                    <tr class="{{ $index % 2 == 0 ? 'bg-gray-100' : '' }}">
                        ...
                        <td class="px-4 py-1">{{ $workOrderContainer->container->container_size }}</td>
                        <td class="px-4 py-1">{{ $workOrderContainer->container->container_type }}</td>
                        <td class="px-4 py-1">{{ $workOrderContainer->is_empty ? 'Yes' : 'No' }}</td>
                        <td class="px-4 py-1">{{ $workOrderContainer->seal ?: '-' }}</td>
                        <td class="px-4 py-1">{{ $workOrderContainer->description ?: '-' }}</td>
                    </tr>
                @empty
                    <tr>
        ...
        <div class="bg-white rounded shadow-sm px-6 py-4 mb-4">
            <div class="mb-2 flex items-center justify-between">
                <div>
                    ...
                    <span class="text-gray-400">{{ __('List of job goods') }}</span>
                </div>
            </div>
            <table class="table-auto w-full mb-4">
                <thead>
                ...
                @forelse ($workOrder->workOrderGoods as $index => $workOrderItem)
                    <tr class="{{ $index % 2 == 0 ? 'bg-gray-100' : '' }}">
                        ...
                        <td class="px-4 py-1">{{ numeric($workOrderItem->unit_quantity) }}</td>
                        <td class="px-4 py-1">{{ $workOrderItem->goods->unit_name }}</td>
                        <td class="px-4 py-1">{{ numeric($workOrderItem->package_quantity) }}</td>
                        <td class="px-4 py-1">{{ $workOrderItem->goods->package_name }}</td>
                        <td class="px-4 py-1">{{ numeric($workOrderItem->weight) }}</td>
                        <td class="px-4 py-1">{{ $workOrderItem->description ?: '-' }}</td>
                    </tr>
                @empty
                    <tr>
