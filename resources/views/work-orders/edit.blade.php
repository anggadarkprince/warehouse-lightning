    ...
    <form action="{{ route('work-orders.update', ['work_order' => $workOrder->id]) }}" method="post" id="form-work-order">
        ...
        <div class="bg-white rounded shadow-sm px-6 py-4 mb-4">
            ...
            <div class="flex flex-wrap mb-3 sm:mb-4">
                ...
                <div class="relative w-full">
                    <p class="form-input">
                        {{ $workOrder->booking->reference_number }} - {{ $workOrder->booking->booking_number }} ({{ $workOrder->booking->customer->customer_name }})
                    </p>
                    <input type="hidden" name="booking_id" id="booking_id" value="{{ $workOrder->booking_id }}">
                </div>
                            ...
                            <select class="form-input" name="user_id" id="user_id">
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" data-custom-properties='{"avatar": "{{ $user->avatar }}", "email": "{{ $user->email }}"}'
                                        {{ old('user_id', $workOrder->user_id) == $user->id ? ' selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
        ...
        <div class="bg-white rounded shadow-sm px-6 py-4 mb-4">
            <div class="mb-3 flex justify-between items-center">
                ...
                <button type="button" class="button-blue button-sm" id="btn-add-container" data-booking-id="{{ $workOrder->booking_id }}" data-source-url="{{ route('bookings.containers.index', ['booking' => $workOrder->booking_id ?: '0']) }}">
                    ADD CONTAINER
                </button>
            </div>
            <table class="table-auto w-full mb-4">
                ...
                <tr class="container-placeholder{{ empty(old('containers', $workOrder->workOrderContainers->toArray())) ? '' : ' hidden' }}">
                    <td colspan="8" class="px-4 py-2">{{ __('No data available') }}</td>
                </tr>
                @foreach(old('containers', $workOrder->workOrderContainers()->with('container')->get()->toArray()) as $index => $container)
        ...
        <div class="bg-white rounded shadow-sm px-6 py-4 mb-4">
            <div class="mb-3 flex justify-between items-center">
                ...
                <button type="button" class="button-blue button-sm" id="btn-add-goods" data-booking-id="{{ $workOrder->booking_id }}" data-source-url="{{ route('bookings.goods.index', ['booking' => $workOrder->booking_id ?: '0']) }}">
                    ADD GOODS
                </button>
            </div>
            <table class="table-auto w-full mb-4">
                ...
                <tr class="goods-placeholder{{ empty(old('goods', $workOrder->workOrderGoods->toArray())) ? '' : ' hidden' }}">
                    <td colspan="9" class="px-4 py-2">{{ __('No data available') }}</td>
                </tr>
                @foreach(old('goods', $workOrder->workOrderGoods()->with('goods')->get()->toArray()) as $index => $item)
